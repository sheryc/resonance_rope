...
def generate_prompt(docs, num_keys=1):
    ...
    information_lines = [
        f"The {order(i + 1)} pass key is {pass_key}. Remember it. {pass_key} is the {order(i + 1)} pass key." for
        i, pass_key in enumerate(pass_keys)]
...
def main(args):
    ...
    for model in tqdm(models, desc="Model", leave=False):
        ...
        for _ in trange(0, args.iterations, desc="Iterations", leave=False):
            ...
            result = {"prompt_text": prompt_text, "start_pos": start_pos, "pass_keys": pass_keys,
