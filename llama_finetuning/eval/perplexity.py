...
def compute_perplexity(
        encodings, model, tokenizer, add_start_token: bool = True, device=None, max_length=None, sliding_window=256,
        truncate=False, aggressive_memory=False, hide_progress=False,
):
    ...
    for model in tqdm(models, desc="Model", leave=False, disable=args.hide_progress):
        ...
        for max_length in tokens:
            ppl = compute_perplexity(model=loaded, tokenizer=tokenizer, encodings=input_texts,
                                     ...
                                     aggressive_memory=args.aggressive_memory, hide_progress=args.hide_progress)[
                'mean_perplexity']
