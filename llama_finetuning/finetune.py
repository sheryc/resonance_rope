...
def main(args):
    ...
    config.resonance_rope = args.resonance_rope
    ...
    try:
        train_dataset = load_dataset(args.dataset,
                                     split=f'train[:{2 * args.max_train_steps * args.batch_size * accelerator.num_processes * args.gradient_accumulate_every + 1}]')
    except:
        try:
            train_dataset = load_dataset(args.dataset + '/data',
                                         split=f'train[:{2 * args.max_train_steps * args.batch_size * accelerator.num_processes * args.gradient_accumulate_every + 1}]')
        except:
    ...
    if args.truncate:
        ...
        train_dataset = train_dataset.map(truncate, desc="Truncating", num_proc=args.num_proc,
                                          cache_file_name=f"./data_cache/train_dataset_{args.max_train_steps}steps_{args.truncate}.arrow",
    ...
    if args.deepspeed:
        ...
        scheduler = DummyScheduler(optim, total_num_steps=args.max_train_steps, warmup_num_steps=args.warmup_steps)
        ...
        if args.lr_schedule == "linear":
            scheduler = get_linear_schedule_with_warmup(
                optim, num_warmup_steps=args.warmup_steps, num_training_steps=args.max_train_steps)
...
if __name__ == "__main__":
    ...
    args.add_argument("--v1", action="store_true")
    args.add_argument("--resonance-rope", action="store_true")
    args.add_argument("--scaling-factor", type=float, default=16.0)
    args.add_argument("--no-scale", action="store_true")
