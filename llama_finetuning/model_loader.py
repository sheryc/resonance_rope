...
def apply_patches(model, args):
    if not args.custom_model and not args.custom_model_together and not args.custom_model_mistral:
        ...
        if args.dynamic_linear:
            ...
            print('Patching Resonance YaRN scaling...')
            if "LlamaForCausalLM" in model.config.architectures:
                patch_llama_for_yarn_scaled_resonance_rotary_embeddings(
                    model, scale=args.resonance_yarn, original_max_position_embeddings=args.original_max_position_embeddings)
            else:
                raise RuntimeError(
                    f"Unsupported architecture {model.config.architectures} for Resonance YaRN")
        elif args.rerope:
            if "LlamaForCausalLM" in model.config.architectures:
