    ...
    def cos_sin(
        ...
        if seq_len != self.seq_len_cached:
            ...
            if seq_len >= self.max_position_embeddings:
                ...
                inv_freq_linear = 1.0 / (scale * (
                            self.base ** (torch.arange(0, self.head_dim, 2).float().to(device) / self.head_dim)))
                inv_freq_ntk = 1.0 / (find_newbase_ntk(self.head_dim, self.base, scale) ** (
                            torch.arange(0, self.head_dim, 2).float().to(device) / self.head_dim))

                # Combine NTK and Linear
                low, high = find_correction_range(beta_0, beta_1, self.head_dim, self.base,
                                                  self.max_position_embeddings)
                inv_freq_mask = (1 - linear_ramp_mask(low, high, self.head_dim // 2).type(dtype).to(
                    device)) * self.ntk_factor
                inv_freq = inv_freq_linear * (1 - inv_freq_mask) + inv_freq_ntk * inv_freq_mask

                # Combine Extrapolation and NTK and Linear
                low, high = find_correction_range(gamma_0, gamma_1, self.head_dim, self.base,
                                                  self.max_position_embeddings)
                inv_freq_mask = (1 - linear_ramp_mask(low, high, self.head_dim // 2).type(dtype).to(
