...
class LlamaDynamicYaRNScaledResonanceRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, max_position_embeddings=2048, base=10000, original_max_position_embeddings=2048,
        ...
        freqs = self.compute_freqs(max_position_embeddings)
    ...
    def forward(self, x, seq_len=None):
        ...
        if seq_len > self.max_seq_len_cached:
            ...
            freqs = self.compute_freqs(seq_len)
            # Different from paper, but it uses a different permutation in order to obtain the same calculation
            emb = freqs
    ...
    def yarn(self, scale, device):
        ...
        self.resonance_register(inv_freq)
        self.mscale = float(
            get_mscale(scale) * self.attn_factor)  # Get n-d magnitude scaling corrected for interpolation
