    ...
    def compute_freqs(self, seq_len, device, dtype=None):
        ...
        for i in range(self.dim // 2):
            if seq_len >= self.r_wavelengths[i].item():
                ...
                t_i = torch.arange(self.max_seq_len_cached, device=device, dtype=dtype)
...
class FlashYaRNRotaryEmbedding(torch.nn.Module):
    ...
    def __init__(self, dim: int, base=10000.0, interleaved=False,
        ...
        self.mscale = float(_yarn_get_mscale(
            self.scaling_factor) * attn_factor)  # Get n-d magnitude scaling corrected for interpolation
...
class FlashResonanceYarnRotaryEmbedding(FlashYaRNRotaryEmbedding, ResonanceEmbedding):
    ...
    def __init__(self, dim: int, base=10000.0, interleaved=False,
                 ...
                 attn_factor=1, beta_fast=32, beta_slow=1,
                 dynamic=False, finetuned=False, device=None):
        super().__init__(dim, base, interleaved, scaling_factor, pos_idx_in_fp32, max_position_embeddings,
                         original_max_position_embeddings, extrapolation_factor, attn_factor, beta_fast, beta_slow,
                         dynamic, finetuned, device)
        if not dynamic:
    ...
    def _update_cos_sin_cache(self, seqlen, device=None, dtype=None):
        ...
        if (seqlen > self._seq_len_cached or self._cos_cached.device != device
            ...
            # However, for compatibility reason, we add an option to use the dtype of self.inv_freq.
            if self.pos_idx_in_fp32:
                freqs = self.compute_freqs(seq_len=seqlen, device=device, dtype=torch.float32)
            else:
                freqs = self.compute_freqs(seq_len=seqlen, device=device, dtype=None)
            self._cos_cached = (torch.cos(freqs) * self.mscale).to(dtype)
            self._sin_cached = (torch.sin(freqs) * self.mscale).to(dtype)
...
class FlashResonanceRotaryEmbedding(FlashRotaryEmbedding, ResonanceEmbedding):
    pass
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        if scaling_type == "resonance-yarn" or scaling_type == "dynamic-resonance-yarn":
            original_max_position_embeddings = self.config.rope_scaling["original_max_position_embeddings"]

            self.rotary_emb = FlashResonanceYarnRotaryEmbedding(
                self.head_dim, base=self.rope_theta, interleaved=False, scaling_factor=scaling_factor,
                max_position_embeddings=self.max_position_embeddings,
                original_max_position_embeddings=original_max_position_embeddings,
                dynamic=scaling_type.startswith("dynamic"), finetuned=self.config.rope_scaling.get("finetuned", False)
        ...
        elif scaling_type == "resonance-linear" or scaling_type == "resonance-ntk":
            self.rotary_emb = FlashResonanceRotaryEmbedding(
                self.head_dim, base=self.rope_theta, interleaved=False, scaling_factor=scaling_factor,
            )
        else:
    ...
    def forward(
        ...
        if is_padded_inputs:
            ...
            attn_outputs = flash_attn_kvpacked_func(
                q, kv, dropout_p=0.0, softmax_scale=1.0 / self.norm_factor, causal=(not has_layer_past),
