...
class MistralYaRNScaledRotaryEmbedding(torch.nn.Module):
    ...
    def _set_cos_sin_cache(self, length):
...
class MistralResonanceYaRNScaledRotaryEmbedding(MistralYaRNScaledRotaryEmbedding, ResonanceEmbedding):
    def _set_cos_sin_cache(self, length):
        self.max_seq_len_cached = length

        freqs = self.compute_freqs(self.max_seq_len_cached, self.r_inv_freq.device, self.r_inv_freq.dtype)
        emb = torch.cat((freqs, freqs), dim=-1)
        dtype = torch.get_default_dtype()
        self.register_buffer("cos_cached", (emb.cos() * self.mscale).to(dtype), persistent=False)
        self.register_buffer("sin_cached", (emb.sin() * self.mscale).to(dtype), persistent=False)
    ...
    def yarn(self, device):
        ...
        self.mscale = float(
            _yarn_get_mscale(
                self.scale) * self.attn_factor)  # Get n-d magnitude scaling corrected for interpolation
