...
def patch_llama_for_dynamic_yarn_rotary_embeddings(model, original_max_position_embeddings, finetuned):
    ...
    for each in model.model.layers:
        each.self_attn.rotary_emb = LlamaDynamicYaRNScaledRotaryEmbedding(
            each.self_attn.head_dim, finetuned=finetuned,
...
def patch_llama_for_yarn_scaled_rotary_embeddings(model, scale, original_max_position_embeddings):
    ...
    for each in model.model.layers:
        each.self_attn.rotary_emb = LlamaYaRNScaledRotaryEmbedding(
            each.self_attn.head_dim, scale=scale, original_max_position_embeddings=original_max_position_embeddings,
            device=each.self_attn.rotary_emb.inv_freq.device)


def patch_llama_for_yarn_scaled_resonance_rotary_embeddings(model, scale, original_max_position_embeddings):
    from .LlamaYaRNScaledResonanceRotaryEmbedding import LlamaYaRNScaledResonanceRotaryEmbedding
    for each in model.model.layers:
        each.self_attn.rotary_emb = LlamaYaRNScaledResonanceRotaryEmbedding(
            each.self_attn.head_dim, scale=scale, original_max_position_embeddings=original_max_position_embeddings,
            device=each.self_attn.rotary_emb.inv_freq.device)
...
def patch_gptneox_for_scaled_rotary_embeddings(model):
    ...
    for each in model.gpt_neox.layers:
        each.attention.rotary_emb = GPTNeoXDynamicScaledRotaryEmbedding(
            each.attention.rotary_ndims, model.config.max_position_embeddings,
            device=each.attention.rotary_emb.inv_freq.device)
...
def patch_gptneox_for_ntk_scaled_rotary_embeddings(model, alpha):
    ...
    for each in model.gpt_neox.layers:
        each.attention.rotary_emb = GPTNeoXNTKScaledRotaryEmbedding(
            each.attention.rotary_ndims, model.config.max_position_embeddings, alpha=alpha,
            device=each.attention.rotary_emb.inv_freq.device)


def patch_gptneox_for_longer_sequences(model, max_positions):
    for each in model.gpt_neox.layers:
        each.attention.bias = torch.tril(torch.ones((max_positions, max_positions), dtype=each.attention.bias.dtype,
                                                    device=each.attention.bias.device)).view(
